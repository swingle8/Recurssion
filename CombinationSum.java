...
public class CombinationSum {
    public static void main(String [] args) {
    	...
    	while (cases --> 0) {
			...
			int sum = sc.nextInt();
			calculate (al, 0, "", sum, 0);
    	}
    }

    
         static void calculate (ArrayList<Integer> A, int index, String ans, int N, int sum) {
            if (index == A.size()) {
            	if (sum == N)
           ...
           calculate (A, index+1, ans, N, sum);
           int num = A.get(index);
           sum = sum + num;
           calculate (A, index+1, ans+num , N, sum);
